...
public class PersonalityRecognizer {
	...
	public Map<String,Double> getFeatureCounts(String text, boolean relativeOnly) throws Exception {
		...
		for (String longFeature : initCounts.keySet()) { 
			...
			if (featureShortcuts.containsKey(longFeature)) {
				...
				System.err.println("Checking: LIWC feature " + longFeature + " has been recognized.");
			} else {
	...
	public void printOutput(Classifier[] models, Map<File,Double[]> scores,
		...
		for(File file : scores.keySet()) {
			...
			for (int j = 0; j < models.length; j++) {
				//out.println("ATENCION : "+ scores.containsKey(file));
	...
	private Map<File,Instance> getCorpusDataset(File dir) {
		...
		System.err.println("Hi, Computing standardized values for each feature over the whole corpus ("+ dataset.numInstances() + " files)");
	...
	private Instance addInstance(Instances dataset, Map<String, Double> counts) {
		...
		//System.err.println("size = "+Collections.list((Enumeration<Attribute>) dataset.enumerateAttributes()).size());
		for (Attribute attr : Collections.list((Enumeration<Attribute>) dataset.enumerateAttributes())) {
			//System.out.println("attr ="+attr.name().toUpperCase()+" and contains ="+counts.containsKey(attr.name().toUpperCase()));
